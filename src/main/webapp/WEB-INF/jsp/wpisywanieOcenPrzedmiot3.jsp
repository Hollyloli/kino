...
<div class="col-sm-12" id="divMain">
	...
	<c:choose>
		<c:when test="${wpisanaOcena=='semestralna'}">
			<h2>Brak możliwości wpisywania/edycji/usunięcia/wystawiania
				oceny.Wystawiono już ocenę semestralną, a kolejny semestr jeszcze
				się nie zaczął.</h2>

		</c:when>
		<c:when test="${wpisanaOcena=='koncowa'}">
			<h2>Brak możliwości wpisywania/edycji/usunięcia/wystawiania
				oceny. Wystawiono już ocenę końcową</h2>
			...
			<c:choose>
				<c:when test="${wybor2=='wpisywanie_oceny'}">
					<form:form commandName="ocena" class="form-horizontal"
						action="/PracaInz/wpisanieOceny.html">
						<div class="form-group">
							<label for="adres" class="control-label">Wybrana uczeń</label> <input
								class="form-control" value="${uczen.imie} ${uczen.nazwisko}"
								disabled="disabled" />
						</div>
						<div class="form-group">
				...
				<c:when
					test="${wybor2=='edycja_oceny' || wybor2=='usuniecie_oceny' || wybor2=='wystawienie_oceny_sem_lub_rocznej'}">
						...
						<table class="table table-bordered">
							<thead>
								<tr>
									...
									<td rowspan="3" align="center"><strong>Imie i
											Nazwisko</strong></td>
								...
								<tr>
									...
									<c:forEach items="${uczen.oceny}" var="pojedynczaOcena"
										...
										<c:if
											...
											<td align="center">${pojedynczaOcena.ocena}<c:choose>
													<c:when test="${wybor2=='edycja_oceny'}">
														<button type="button" class="btn btn-primary btn-xs"
															data-toggle="modal" data-target="#exampleModal"
															data-whatever="${uczen.oceny[loop.index].idOceny}">Zmien</button>
													</c:when>
													<c:when test="${wybor2=='usuniecie_oceny'}">
														<a
															href="<spring:url value="/usuniecieOceny/${uczen.oceny[loop.index].idOceny}.html"  /> "
															class="btn btn-danger btn-xs">usuń ocene</a>
													</c:when>
												</c:choose>
											</td>
									...
									<c:if test="${wybor2=='wystawienie_oceny_sem_lub_rocznej'}">
										<td align="center"><c:forEach items="${uczen.oceny}"
												var="pojedynczaOcena">
							...
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									<div class="modal-header">
										...
										<h4 class="modal-title" id="exampleModalLabel">Panel
											zmiany oceny</h4>
									</div>
					...
					<form:form commandName="ocena"
						action="/PracaInz/wystawienieOcenyKoncowej.html">
							...
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									...
									<div class="modal-body">
										...
										<div class="form-group">
											<label for="labelWyborKlasy">Średnia ucznia</label> <input
												class="form-control" value="${sredniaOcen}"
												disabled="disabled" />
										</div>
										...
										<c:choose>
											...
											<c:when
												test="${sredniaPoZanizeniu>=1.75 && sredniaPoZanizeniu<2.75}">
												<c:set var="ocenaKoncowaPoZanizeniu" value="2" />
											</c:when>
											<c:when
												test="${sredniaPoZanizeniu>=2.75 && sredniaPoZanizeniu<3.75}">
												<c:set var="ocenaKoncowaPoZanizeniu" value="3" />
											</c:when>
											<c:when
												test="${sredniaPoZanizeniu>=3.75 && sredniaPoZanizeniu<4.75}">
												<c:set var="ocenaKoncowaPoZanizeniu" value="4" />
											</c:when>
											<c:when
												test="${sredniaPoZanizeniu>=4.75 && sredniaPoZanizeniu<5.4}">
												<c:set var="ocenaKoncowaPoZanizeniu" value="5" />
											</c:when>
											<c:when
												test="${sredniaPoZanizeniu>=5.4 && sredniaPoZanizeniu<6}">
										...
										<c:choose>
											...
											<c:when
												test="${sredniaPoZawyzeniu>=1.75 && sredniaPoZawyzeniu<2.75}">
												<c:set var="ocenaKoncowaPoZawyzeniu" value="2" />
											</c:when>
											<c:when
												test="${sredniaPoZawyzeniu>=2.75 && sredniaPoZawyzeniu<3.75}">
												<c:set var="ocenaKoncowaPoZawyzeniu" value="3" />
											</c:when>
											<c:when
												test="${sredniaPoZawyzeniu>=3.75 && sredniaPoZawyzeniu<4.75}">
												<c:set var="ocenaKoncowaPoZawyzeniu" value="4" />
											</c:when>
											<c:when
												test="${sredniaPoZawyzeniu>=4.75 && sredniaPoZawyzeniu<5.4}">
												<c:set var="ocenaKoncowaPoZawyzeniu" value="5" />
											</c:when>
											<c:when
												test="${sredniaPoZawyzeniu>=5.4 && sredniaPoZawyzeniu<6}">
												<c:set var="ocenaKoncowaPoZawyzeniu" value="6" />
											</c:when>
										</c:choose>
										<div class="form-group">
											<label for="labelWyborKlasy">Średnia ucznia po
												zawyżeniu o 0.5</label> <input class="form-control"
												value="${sredniaPoZawyzeniu}" disabled="disabled" />
										</div>
										<div class="form-group">
											<label for="labelWyborKlasy">Średnia ucznia po
												zaniżeniu o 0.5</label> <input class="form-control"
												value="${sredniaPoZanizeniu}" disabled="disabled" />
										</div>
										<div class="form-group">
											<label for="labelWyborKlasy">Proponowana ocena</label> <input
												class="form-control" value="${ocenaKoncowa}"
												disabled="disabled" />
										</div>
										<div class="form-group">
											<label for="labelWyborKlasy">Wybór oceny końcowej
												uwzgledniając zmiane średniej</label>
											<form:select path="ocena" class="form-control">
