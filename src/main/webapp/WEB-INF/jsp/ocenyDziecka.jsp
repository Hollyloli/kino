...
<div class="col-sm-12" id="divMain">
		...
		<form:form commandName="ocena" class="form-horizontal" action="/PracaInz/formularzOcenDziecka.html">
			...
			<c:if test="${ocenySemestr != null}">
				...
				<c:forEach items="${ocenySemestr}" var="przedmiot" varStatus="loop">
               		...
               		<c:forEach items="${przedmiot.value}" var="pojOcena" >
               			<c:choose>
               				<c:when test="${(pojOcena.wagaOceny==1) && pojOcena.typ=='czastkowa'}">
               					<c:set var="TliczbaOcenWag1" value="${TliczbaOcenWag1+1}"></c:set>
               				</c:when>
               				<c:when test="${(pojOcena.wagaOceny==2) && pojOcena.typ=='czastkowa'}">
               					<c:set var="TliczbaOcenWag2" value="${TliczbaOcenWag2+1}"></c:set>
               				</c:when>
               				<c:when test="${(pojOcena.wagaOceny==3) && pojOcena.typ=='czastkowa'}">
		                    	...
		                    	<thead>
		        				 	<tr>
		                              <td rowspan="3" align="center" ><strong>lp.</strong></td>
		                              	...
		                              	<td rowspan="3" align="center"><strong>Nazwa przedmiotu</strong></td>
								...
								<tbody>
										<c:forEach items="${ocenySemestr}" var="przedmiot" varStatus="loop">
									 		<tr>
	                                    		...
	                                    		<td align="center">
		                                    		<c:forEach items="${przedmiot.value}" var="pojedynczaOcena">
	                           							<c:if test="${pojedynczaOcena.typ=='semestralna' || pojedynczaOcena.typ=='koncowa'}">
