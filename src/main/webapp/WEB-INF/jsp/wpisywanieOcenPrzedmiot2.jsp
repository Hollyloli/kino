	...
	<c:choose>
		<c:when test="${wybor=='czynnosci_zwiazane_z_edycja_ocen'}">
			...
			<c:choose>
				<c:when test="${ocenaKoncowa==null}">
					<form:form commandName="wspolne" class="form-horizontal"
						action="/PracaInz/formularzUczenIczynnnosc.html">
						<div class="form-group">
							<label for="adres" class="control-label">Wybrana klasa</label> <input
								class="form-control" value="${klasa.rok} ${klasa.symbol}"
								disabled="disabled" />
						</div>
						<div class="form-group">
							<label for="labelWyborKlasy">Wybor ucznia</label>
							<form:select path="uczen.imie" class="form-control">
								<c:forEach items="${klasa.uczniowie}" var="uczen"
									varStatus="loop">
				...
				<c:when test="${ocenaKoncowa!=null}">
					<h2>Wystawiono już końcowe ocene dla wszystkich uczniów. Opcja
						czynności związane z edycją ocen jest niedostępna.</h2>
				</c:when>
			</c:choose>
		</c:when>
		...
		<c:when test="${wybor=='czynnosci_zwiazane_z_podgladaniem_ocen'}">
			<form:form commandName="ocena" class="form-horizontal"
				action="/PracaInz/formularzWyswietlenieOcen.html">
			...
			<c:if test="${ocenySemestr1 != null}">
				...
				<c:forEach items="${ocenySemestr1.uczniowie}" var="uczen2"
					varStatus="loop">
					...
					<c:forEach items="${uczen2.oceny}" var="pojedynczaWaga">
						<c:choose>
							<c:when
								test="${(pojedynczaWaga.wagaOceny==1) && pojedynczaWaga.typ=='czastkowa'}">
								<c:set var="TliczbaOcenWag1" value="${TliczbaOcenWag1+1}"></c:set>
							</c:when>
							<c:when
								test="${(pojedynczaWaga.wagaOceny==2) && pojedynczaWaga.typ=='czastkowa'}">
								<c:set var="TliczbaOcenWag2" value="${TliczbaOcenWag2+1}"></c:set>
							</c:when>
							<c:when
								test="${(pojedynczaWaga.wagaOceny==3) && pojedynczaWaga.typ=='czastkowa'}">
						...
						<tbody>
							<c:forEach items="${ocenySemestr1.uczniowie}" var="uczen2"
								...
								<tr>
									...
									<td align="center"><c:forEach items="${uczen2.oceny}"
											var="pojedynczaOcena">
			...
			<c:if test="${ocenySemestr2 != null}">
				...
				<c:forEach items="${ocenySemestr2.uczniowie}" var="uczen2"
					varStatus="loop">
					<c:set var="TliczbaOcenWag1" value="${0}" />
					<c:set var="TliczbaOcenWag2" value="${0}" />
					<c:set var="TliczbaOcenWag3" value="${0}" />
					...
					<table class="table table-bordered">
						<thead>
							<tr>
								...
								<td align="center" rowspan="3"><strong>Imie i
										Nazwisko</strong></td>

								<td align="center" colspan="${maxLiczbaOcen}"><strong>Oceny
										i wagi</strong></td>
		...
		<c:when
			test="${wybor=='czynnosci zwiazane z podgladaniem frekfencji'}">
		</c:when>
	</c:choose>
